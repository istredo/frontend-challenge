...
const Cat: React.FC<ICats> = ({ url, id }) => {
	...
	const onLike = () => {
		...
		if (svgRef.current) {
			svgRef.current.style.transform = 'scale(1.2)';
		}
	}
	return (
		<div className="cats__item" key={id} >
			<div className="cats__bg"></div>
			<div className="gradient">	</div>
			<img className='cats__img' src={url} alt="awesome cat" />
			<button className='button' key={url} onClick={onLike}>
				<svg className='svg' width="40" height="37" viewBox="0 0 40 37" fill="none" xmlns="http://www.w3.org/2000/svg">
				...
				<svg ref={svgRef} className='svg-liked' width="40" height="37" viewBox="0 0 40 37" fill="none" xmlns="http://www.w3.org/2000/svg">
